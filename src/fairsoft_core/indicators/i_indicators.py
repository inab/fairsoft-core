...
def compI1_1(instance) -> Tuple[bool, List[str]]:
    ...
    for input in inputs:
        if input.vocabulary:
            logs.append(
                f"✅ Input '{input.term}' uses a standard data format '{input.vocabulary}'."
            )
        else:
    ...
    for output in outputs:
        if output.vocabulary:
            logs.append(
                f"✅ Output '{output.term}' uses a standard data format '{output.vocabulary}'."
            )
        else:
...
def compI1_2(instance) -> Tuple[bool, List[str]]:
    ...
    logs.append(
        "⚙️ Checking if any documentation entry is an API specification and the url is operational."
    )
    ...
    for doc in documentation:
        if doc.type.lower() == "api specification":
            ...
            if is_url_operational(doc.url):
                logs.append(
                    "✅ API specification found in documentation and the URL is operational."
                )
                logs.append("Result: PASSED")
                return True, logs
            logs.append(
                f"❌ API specification found in documentation but the URL is not operational. URL: {doc.url}."
            )
...
def compI1_3(instance) -> Tuple[bool, List[str]]:
    ...
    logs.append(
        "Formats considered verifiable: https://observatory.openebench.bsc.es/api/lists/verifiable_formats"
    )
...
def compI2_1(instance) -> Tuple[bool, List[str]]:
    ...
    logs.append(
        "⚙️ Checking if the instance type is one of the valid options (lib, rest, soap, or api)."
    )
...
def compI2_2(instance) -> Tuple[bool, List[str]]:
    ...
    logs.append(
        "⚙️ Checking if at least one e-infrastructure is referenced in the links and the link is operational"
    )
    logs.append(
        "Considered e-infrastructures: https://observatory.openebench.bsc.es/api/lists/e_infrastructures"
    )
    ...
    if webpage:
        ...
        if n_operational >= 1:
            ...
            logs.append(
                "❌ No operational e-infrastructures referenced in the links. Checking sources ..."
            )
...
def compI3_2(instance) -> Tuple[bool, List[str]]:
    ...
    # Check registries
    logs.append("-- Checking registries")
    logs.append(f"Registries: {registries}")
    ...
    # Check links
    logs.append("-- Checking links")
    logs.append(f"Links: {links}")
