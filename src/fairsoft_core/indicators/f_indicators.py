...
def compF1_2(instance) -> Tuple[bool, str]:
    ...
    if instance.version:
        logs.append(
            f"⚙️ Checking if versions '{instance.version}' follows semantic versioning standards."
        )
        ...
        for version in instance.version:
            ...
            if len(parts) < 2:
                logs.append(
                    "❌ Version does not have enough parts (should be at least MAJOR.MINOR)."
                )
...
def compF2_1(instance) -> Tuple[bool, str]:
    """Structured Metadata."""
    logs = []
    logs.append("⚙️ Checking if any source provides structured metadata.")
    ...
    logs.append("⚙️ Comparing sources against: ")
...
def compF2_2(instance) -> Tuple[bool, str]:
    ...
    logs = []
    logs.append(
        "⚙️ Verifying whether a topic or operation is part of a specific ontology or controlled vocabulary."
    )
    logs = log_topics_operations(instance, logs)

    if instance.topics:
        logs.append("⚙️ Checking if any topic is part of an ontology or controlled vocabulary.")
        for topic in instance.topics:
            if topic.vocabulary:
                logs.append(
                    f"✅ Topic '{topic.term}' is part of the '{topic.vocabulary}' ontology or controlled vocabulary."
                )
                logs.append("Result: PASSED")
                return True, logs
            else:
                logs.append(
                    f"❌ Topic '{topic.term}' is not part of any ontology or controlled vocabulary."
                )

    if instance.operations:
        logs.append("⚙️ Checking if any operation is part of an ontology or controlled vocabulary.")
        for operation in instance.operations:
            if operation.vocabulary:
                logs.append(
                    f"✅ Operation '{operation.term}' is part of the '{operation.vocabulary}' ontology or controlled vocabulary."
                )
                logs.append("Result: PASSED")
                return True, logs
            else:
                logs.append(
                    f"❌ Operation '{operation.term}' is not part of any ontology or controlled vocabulary."
                )

    logs.append(
        "❌ No topic or operation involves the use of ontologies or controlled vocabularies."
    )
    logs.append("Result: FAILED")
    return False, logs
...
def compF3_1(instance) -> Tuple[bool, str]:
    ...
    logs = []
    logs.append("⚙️ Checking if any source matches software registries.")
    logs = log_sources(instance, logs)
    ...
    if instance.source is None:
        ...
        logs.append("❌ No source matches software registries. Checking registries ...")

    logs.append(
        "⚙️ Checking if the specified software registry matches any of the known registries."
    )
    ...
    if instance.registries is None:
        ...
        logs.append("⚙️ Comparing registries against: ")
        ...
        logs.append("❌ No registry matches software registries.")
        logs.append("Result: FAILED")
        return False, logs
...
def compF3_2(instance) -> Tuple[bool, str]:
    ...
    logs.append("⚙️ Checking if any repository is an operational software repository.")
...
def compF3_3(instance) -> Tuple[bool, str]:
    ...
    logs.append("⚙️ Checking if any publication is a valid publication.")
