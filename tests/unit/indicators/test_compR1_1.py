...
from fairsoft_core.indicators.r_indicators import compR1_1
...
def test_compR1_1_with_usage_guide_no_op_url():
    instance = Instance(
        documentation=[Documentation(type="usage guide", url="https://githubbbb.com")]
    )
    result, logs = compR1_1(instance)
    assert result == False
...
def test_compR1_1_with_no_guide_docs():
    instance = Instance(
        documentation=[Documentation(type="license"), Documentation(type="terms of use")]
    )
    result, logs = compR1_1(instance)
    assert result == False
