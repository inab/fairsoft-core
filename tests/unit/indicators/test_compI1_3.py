...
from fairsoft_core.indicators.i_indicators import compI1_3
...
def create_instance(input_data, output_data):
    return Instance(input=input_data, output=output_data)
...
def test_compI1_3_with_vocabulary_in_input_and_output():
    instance = create_instance(
        [{"vocabulary": "EDAM", "term": "JSON"}], [{"vocabulary": "EDAM", "term": "CSV"}]
    )
    result, logs = compI1_3(instance)
    assert result == True
