...
from fairsoft_core.indicators.f_indicators import compF2_2
from fairsoft_core.models.instance import ControlledTerm, Instance
...
def test_compF2_2_with_mixed_vocabulary():
    topics = [
        ControlledTerm(vocabulary="EDAM", term="genomics"),
        ControlledTerm(vocabulary="", term="random_topic"),
    ]
    operations = [
        ControlledTerm(vocabulary="EDAM", term="variant calling"),
        ControlledTerm(vocabulary="", term="random_operation"),
    ]
    instance = Instance(topics=topics, operations=operations)
    result, logs = compF2_2(instance)
    assert result == True
